...
export function TasksSection() {
    ...
    return (
        <article className="flex min-h-0 flex-col rounded-[1.75rem] bg-card p-6 shadow-md ring-1 ring-border/40 lg:p-7">
            <div className="mb-6 flex shrink-0 items-center justify-between gap-3">
                ...
                {completedCount > 0 && (
                    <button
                        ...
                        className="text-xs text-muted-foreground hover:text-destructive font-medium transition-colors border border-border/60 bg-background px-2.5 py-1 rounded-lg shadow-sm"
                    >
            ...
            <div className={cn("min-h-0 flex-1", todos && todos.length > 3 && "max-h-64 overflow-y-auto pr-1")}>
                {!todos || todos.length === 0 ? (
                    <p className="text-sm text-muted-foreground">No focus items yet. Add a task below to get started.</p>
                ) : (
                        ...
                        {todos.map((todo) => {
                            ...
                            return (
                                <TaskItem
                                    ...
                                    onStartEdit={() => startEditTodo(todo.id, todo.label, todo.startDate || "", todo.dueDate || "")}
                                    onCancelEdit={() => setEditingId(null)}
                                    onCommitEdit={commitEditTodo}
                                    onDecrement={() => updateTodo(todo.id, { progress: Math.max((todo.progress || 0) - 10, 0) })}
                                    onIncrement={() => updateTodo(todo.id, { progress: Math.min((todo.progress || 0) + 10, 100) })}
                                />
                            )
                        })}
                    </ul>
            ...
            <div className="mt-4 shrink-0 border-t border-border/50 pt-4 space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-3 bg-muted/40 p-2.5 rounded-xl border border-border/30">
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        ...
                        <span>Start:</span>
                        <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="bg-card border border-border/60 rounded px-1.5 py-0.5 text-foreground text-xs" />
                    </div>
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        <Calendar className="size-3.5 text-destructive/70" />
                        <span>Finish:</span>
                        <input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="bg-card border border-border/60 rounded px-1.5 py-0.5 text-foreground text-xs" />
                    </div>
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground flex-1 min-w-[10rem] justify-end">
                        ...
                        <input type="range" min="0" max="100" step="10" value={progress} onChange={(e) => setProgress(parseInt(e.target.value))} className="w-16 h-1 appearance-none bg-border rounded-lg accent-primary" />
                        <span className="font-mono text-[11px] min-w-[1.5rem] text-right">{progress}%</span>
                    </div>
                    <Button type="button" variant="ghost" size="sm" className="gap-1.5 text-primary text-xs" onClick={addTask}>
